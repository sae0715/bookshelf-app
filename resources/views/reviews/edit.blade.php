                    ...
                    <form action="{{ route('reviews.update', $review) }}" method="POST" novalidate>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex flex-row-reverse gap-1">
                                @for($i = 5; $i >= 1; $i--)
                                <input type="radio" id="rating-{{ $i }}" name="rating" value="{{ $i }}" class="sr-only peer" {{ old('rating', $review->rating) == $i ? 'checked' : '' }} required>
                                <label for="rating-{{ $i }}" class="cursor-pointer text-2xl text-gray-300 peer-checked:text-yellow-400 hover:text-yellow-400">★</label>
                                @endfor
                            </div>
                            @error('rating')
